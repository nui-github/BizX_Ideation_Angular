...
interface LayoutProps {
  ...
  // Which team the signed-in user is currently acting as — switchable from the profile menu so
  // QA can preview team-scoped views (shipment presets, OCR schema ownership, ...) without a
  // real multi-account login. Defaults to Operation.
  currentTeam?: string;
...
export const Layout: React.FC<LayoutProps> = ({ children, currentUserRole, onToggleRole, language, onLanguageChange, onNavigate, onNotificationClick, currentTeam = 'operation', onTeamChange }) => {
  const currentTeamLabel = MOCK_TEAMS.find(t => t.value === currentTeam)?.label || currentTeam;
  const [collapsed, setCollapsed] = useState(false);
  const [profileOpen, setProfileOpen] = useState(false);
  const profileMenuRef = useRef<HTMLDivElement>(null);

  // Click-outside close instead of onBlur — onBlur fires the moment focus leaves the toggle
  // button (e.g. the instant the team-switch <select> below gets clicked), which used to close
  // this whole menu before a native <select> popup even had a chance to register a choice.
  useEffect(() => {
            ...
            >
              <div className="hidden sm:flex flex-col items-end">
                 ...
                 <div className="flex items-center gap-1.5 text-[10px] text-slate-500 font-bold uppercase tracking-wider mt-0.5">
                   ...
                   <span className="text-[#1f5df9] font-black">{language === 'TH' ? `ทีม ${currentTeamLabel.toUpperCase()}` : `${currentTeamLabel.toUpperCase()} TEAM`}</span>
                 </div>
              </div>
              ...
              <div className="absolute right-0 mt-2 w-56 bg-white rounded-xl shadow-lg border border-slate-100 py-1 z-[1200] animate-in fade-in zoom-in-95 duration-100">
                <div className="px-4 py-3 border-b border-slate-100">
                  ...
                  <div className="text-xs text-slate-500 flex flex-col gap-0.5 mt-1">
                    ...
                    <span className="text-[#1f5df9] font-bold">{language === 'TH' ? `ทีม: ${currentTeamLabel}` : `Team: ${currentTeamLabel}`}</span>
                  </div>
                </div>
                <div className="py-1">
                  ...
                  <div className="border-t border-slate-100 py-1">
                    <div className="flex items-center justify-between gap-2 px-4 py-2">
                       <div className="flex items-center gap-2 text-sm text-slate-600 shrink-0">
                          ...
                          <span>{language === 'TH' ? 'สลับทีม' : 'Switch team'}</span>
                       </div>
                       <select
                         value={currentTeam}
                         onChange={(e) => onTeamChange(e.target.value)}
                         className="min-w-0 bg-slate-100 rounded-[4px] px-2 py-1 text-xs font-bold text-slate-600 outline-none focus:ring-2 focus:ring-[#1f5df9]/20 cursor-pointer"
                       >
                         {MOCK_TEAMS.map(team => (
